...
This document describes how to use the functions exposed by `GtPlatform.h` across supported systems (Windows & Linux).
...
All platform differences are handled inside the corresponding `.c` file. You only include `GtPlatform.h`.

```c
#include <GONTI/Platform/GtPlatform.h>
```
...
```c
void* ptr = gontiPlatformAllocate(256, GtFalse);
```
...
```c
GtF64 now = gontiPlatformGetAbsoluteTime();
```
...
GtF64 freq = gontiPlatformGetClockFrequency();
```
