...
Compile-time flags (`GT_LOG_WARN_ENABLED`, etc.) enable or disable categories. In release builds (`KRELEASE == 1`), `DEBUG` and `TRACE` logs are automatically turned off.
...
2. The buffer is zeroed with `gt_setMemory`.
...
Messages with level `< GT_LOG_LEVEL_WARN` (i.e., fatal and error) are sent to:
