...
2. Allocates `length + 1` bytes using `gt_allocate()` with the tag `GT_MEM_TAG_STRING`.
3. Copies the string and the null terminator using `gt_copyMemory()`.
...
* Returns `GtTrue` when both strings are identical.
* Returns `GtFalse` otherwise.
...
* Memory is allocated via `gt_allocate()`.
* Memory must be freed using `gt_free()`.
* Tagging ensures string memory usage appears correctly in `gontiGetMemoryUsageStr()`.
