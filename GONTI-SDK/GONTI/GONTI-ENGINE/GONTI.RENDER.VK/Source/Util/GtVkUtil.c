...
/**
 * @brief Function created by Travis Vroman. KOHI GAME ENGINE
 * https://github.com/travisvroman/kohi/blob/main/kohi.plugin.renderer.vulkan/src/gontiVk_utils.c
 ...
 * @return const char* 
 */
const char* gontiVkUtilResultToString(VkResult result, GtB8 get_extended) {
    // Success Codes
    switch (result) {
        default:
        case VK_SUCCESS:
        ...
        return !get_extended ? "VK_PIPELINE_COMPILE_REQUIRED_EXT" : "VK_PIPELINE_COMPILE_REQUIRED_EXT A requested pipeline creation would have required compilation, but the gontiVkApplication requested compilation to not be performed.";
        ...
        case VK_ERROR_OUT_OF_DATE_KHR:
            ...
            return !get_extended ? "VK_ERROR_INVALID_SHADER_NV" : "VK_ERROR_INVALID_SHADER_NV One or more shaders failed to compile or link. More details are reported back to the gontiVkApplication via VK_EXT_debug_report if enabled.";
 ...
 * @return GtB8 
 */
GtB8 gontiVkUtilResultIsSuccess(VkResult result) {
    switch (result) {
        // Success Codes
        case VK_SUCCESS:
        case VK_NOT_READY:
