...
1. **Header** (3 × `u64` fields):

   * `GONTI_DARRAY_CAPACITY`
   * `GONTI_DARRAY_LENGTH`
   * `GONTI_DARRAY_STRIDE` (sizeof element)
...
* Returns pointer to the first element (header offset by `GONTI_DARRAY_FIELD_LENGTH`).
...
* Shifts all elements right using `k_copyMemory`.
* Writes new value.
* Increments length.
...
* Allocations use GONTI’s custom subsystem (`k_allocate`, `k_free`).
* Tagged with `GONTI_MEMORY_TAG_DARRAY` for statistics.
